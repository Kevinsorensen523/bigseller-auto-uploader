...
- `run_ui.py` (root) — entry point khusus build `.exe` (PyInstaller), bukan dipakai lewat `python -m`. Set `PLAYWRIGHT_BROWSERS_PATH=0` & `BIGSELLER_PROJECT_DIR` sebelum import `webapp` supaya browser bundled & data folder kebaca relatif ke lokasi `.exe`.
- `.github/workflows/build-windows-exe.yml` — build `.exe` Windows otomatis di GitHub Actions (runner `windows-latest`) tiap push ke `main`, bundle Chromium Playwright di dalamnya (`PLAYWRIGHT_BROWSERS_PATH=0` + `--add-data`). Hasil `.exe` didownload dari tab **Actions** run terkait, bagian **Artifacts** (`bigseller-uploader-windows`) — cocok buat PC admin Windows yang gak ada Python.
- `video_compress.py` — `maybe_compress_video(path)`: auto-kompres video job di web UI kalau size > `VIDEO_MAX_SIZE_MB` (default 25MB) atau resolusi > `VIDEO_MAX_DIMENSION` (default 1920px sisi terpanjang), pakai `ffmpeg` (downscale + CRF 26, overwrite di tempat). Dipanggil dari `webapp.py` `submit()` setelah video disimpan. Butuh `ffmpeg`+`ffprobe` di PATH — kalau tidak ada, video dibiarkan apa adanya dan user diberi flash warning. Nonaktifkan lewat `.env` `VIDEO_AUTO_COMPRESS=false`.
